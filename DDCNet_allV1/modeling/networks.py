...
class GAU(nn.Module):
    def __init__(self, channels_high, channels_low, output_shape, upsample=False):
        super(GAU, self).__init__()
        # Global Attention Upsample
        self.upsample = upsample
        self.conv3x3 = nn.Conv2d(channels_low, output_shape, kernel_size=3, padding=1, bias=False)
        self.bn_low = nn.BatchNorm2d(output_shape)

        self.conv1x1 = nn.Conv2d(channels_high, output_shape, kernel_size=1, padding=0, bias=False)
        self.bn_high = nn.BatchNorm2d(output_shape)

        if upsample:
            self.conv_upsample = nn.ConvTranspose2d(channels_high, channels_low, kernel_size=4, stride=2, padding=1, bias=False)
            self.bn_upsample = nn.BatchNorm2d(channels_low)
        else:
            self.conv_reduction = nn.Conv2d(channels_high, output_shape, kernel_size=1, padding=0, bias=False)
            self.bn_reduction = nn.BatchNorm2d(output_shape)
        self.relu = nn.ReLU(inplace=True)
